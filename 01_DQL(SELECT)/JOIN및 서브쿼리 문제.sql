...
SELECT *
FROM (
SELECT EMP_ID, EMP_NAME, JOB_NAME, HIRE_DATE, ROW_NUMBER() OVER (ORDER BY SALARY+SALARY*NVL(BONUS,0) DESC)
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE)
...
--11
--11-1부서 별 급여 합계가 전체 급여 총 합의 20%보다 많은 부서의 부서 명, 부서 별 급여 합계 조회 
...
SELECT * 
FROM (SELECT DEPT_TITLE, SUM(SALARY) AS 월급합
        FROM EMPLOYEE
        JOIN DEPARTMENT ON(DEPT_CODE= DEPT_ID)
        GROUP BY DEPT_TITLE
